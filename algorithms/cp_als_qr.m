function [P,Uinit,output] = cp_als_qr(X,R,varargin)
    ...
    %
    %   M = cp_als_qr(X,R) computes an estimate of the best rank-R
    ...
    %
    %   M = cp_als_qr(X,R,'param',value,...) specifies optional parameters and
    ...
    %
    %   [M,U0] = cp_als_qr(...) also returns the initial guess.
    %
    %   [M,U0,out] = cp_als_qr(...) also returns additional output that contains
    ...
    %Used in 'EFFICIENT CP ROUNDING USING ALTERNATING LEAST SQUARES WITH QR DECOMPOSITION'
